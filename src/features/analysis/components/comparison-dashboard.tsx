...
function ComparisonColumn({ analysis, sideLabel }: { analysis: ComparisonAnalysis; sideLabel: string }) {
  return (
    <Card>
      ...
      <h2 className="mt-4 text-3xl font-semibold text-[var(--foreground)]">{analysis.title}</h2>
          ...
          <div className="mt-3 space-y-3">
            {analysis.suggestions.slice(0, 3).map((suggestion) => (
              <div key={suggestion.title} className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full border border-[var(--border-soft)] bg-[var(--surface-3)] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-[var(--muted-foreground)]">
                  {suggestion.priority}
                </span>
              </div>
            ))}
